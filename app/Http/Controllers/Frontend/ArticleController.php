...
class ArticleController extends Controller
{
    public function getArticle(Request $request,$path,$id)
    {
        ...
        if ($thistypeinfo->mid==1)
        {
            ...
            return view('frontend.brand_article',compact('thisarticleinfos','pics','brandnews','topbrands','latesnews','indexname','latestbrands','cbrands','cnewslists'));
        }else{
            $published=$thisarticleinfos->created_at;
            ...
            $cnewslists=Archive::where('flags','like','%c%')->take(10)->orderBy('id','desc')->get();
            return view('frontend.article_article',compact('thisarticleinfos','prev_article','next_article','flashlingshibrands','latestnewslists','brandnews','hotnewslists','cnewslists'));
